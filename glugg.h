...
// 231001: Simplified the code by adding function saveCurrentData/applySavedData/copySavedData topackage repeated code
// 240908: Minor correction fixing a bug in the above change. Version 1.3.1.
...
// gluggVertexInt is internal, creates arrays of vertices.
...
// Saved state, needed for temporarily setting it while processing saved vertices
...
static void applySavedData(int x)
{
	gluggNormalv(savedNormal[x]);
	gluggTexCoord(savedTexCoord[x].x, savedTexCoord[x].y);
	gluggColorv(savedColor[x]);
	gluggVertexInt(savedVertex[x].x, savedVertex[x].y, savedVertex[x].z);
}

static void copySavedData(int from, int to)
{	
	savedVertex[to] = savedVertex[from];
	savedNormal[to] = savedNormal[from];
	savedColor[to] = savedColor[from];
	savedTexCoord[to] = savedTexCoord[from];
}

static void saveCurrentData(int x, float vx, float vy, float vz)
{
	savedVertex[x] = SetVector(vx,vy,vz);
	savedNormal[x] = SetVector(currentNormalsX,currentNormalsY,currentNormalsZ);
	savedColor[x] = SetVector(currentColorR,currentColorG,currentColorB);
	savedTexCoord[x].x = currentTexCoordS;
	savedTexCoord[x].y = currentTexCoordT;
}
...
//			// Triangle 0-2-3
	...
	if (mode == GLUGG_TRIANGLE_FAN)
	{
		...
		{
			// Save center
			saveCurrentData(0, x, y, z); // Commented out?
// GLUGGW had problems without it.
		}
